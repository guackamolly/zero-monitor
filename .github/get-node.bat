...
:launcher
  "%launcher_bin_path%" ^
    --name="zero-monitor-node" ^
    --description="Service for lauching zero-monitor monitoring agent at startup" ^
    --user="%user%" ^
...
:schedule_launcher
  ...
  if %errorlevel% EQU 60 (
      echo Couldn't schedule a system-service for launching node agent at startup because no supported service manager exists.
  )
